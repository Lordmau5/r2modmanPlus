...
import { disableModsFromActiveProfile } from '../../../utils/Profile';
...
export default class LocalModCard extends Vue {
    ...
    async disableMod() {
        ...
        try {
            await disableModsFromActiveProfile([this.mod]);
        } catch (e) {
            this.$emit('error', e);
...
const dependencyStringToModName = (x: string) => x.substring(0, x.lastIndexOf('-'));
</script>
